...
@Component({
    selector: 'game-page',
    standalone: true,
    imports: [CommonModule, TuiCarouselModule, GameCardComponent, GamePagerComponent, ButtonComponent],
...
export class GamePageComponent implements AfterViewInit {
    ...
    private handleSwipe(): void {
        ...
        document.documentElement.style.setProperty(
            ...
            `${swiperEl.getBoundingClientRect().height}px`,
        );

        swiperEl.addEventListener('swiperslidechange', event => {
            const [swiper] = (event as any).detail;
            console.log('s', swiper.activeIndex);
